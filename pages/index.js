...
// Per-project client-side focal person shown alongside the PFAS team.
// Add entries here as more projects confirm their representatives.
const CLIENT_REPRESENTATIVES = {
  "pcmmdc": {
...
function ClientRepCard({ projectSlug }) {
  const rep = CLIENT_REPRESENTATIVES[projectSlug];
  if (!rep) return null;
  ...
  return (
    <div className="member-card" style={{ display: "flex", flexDirection: "column", gap: 10, padding: 16, background: "linear-gradient(135deg,#FFFBEB,#FFFFFF)", border: "1px solid #FDE68A", borderRadius: 14, minWidth: 0 }}>
      <div style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: 0.6, textTransform: "uppercase", color: "#B45309" }}>Client Representative</div>
      <div style={{ display: "flex", alignItems: "center", gap: 12, minWidth: 0 }}>
        <div style={{ flexShrink: 0, width: 44, height: 44, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 700, fontSize: 15, background: "#B45309" }}>
          {initials}
        </div>
        <div style={{ minWidth: 0, flex: 1 }}>
          <div className="member-name" style={{ fontWeight: 700, fontSize: 15, color: "#1E293B", lineHeight: 1.25 }}>{rep.name}</div>
          <div className="member-role" style={{ fontSize: 12.5, color: "#64748B", lineHeight: 1.35, marginTop: 2 }}>{rep.designation} · {rep.organization}</div>
        </div>
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 5, paddingTop: 11, borderTop: "1px solid #FEF3C7" }}>
        {rep.email && (
          <div className="member-email" style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12.5, minWidth: 0 }}>
            <span style={{ flexShrink: 0, display: "flex", alignItems: "center" }}>
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <rect x="2" y="4" width="20" height="16" rx="3" fill="#B45309" fillOpacity="0.12" stroke="#B45309" strokeWidth="1.6"/>
                <path d="M2 8l10 7 10-7" stroke="#B45309" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
            </span>
            <a href={`mailto:${rep.email}`} style={{ color: "#B45309", textDecoration: "none", overflowWrap: "anywhere", wordBreak: "break-word" }}>{rep.email}</a>
          </div>
        )}
        {rep.contact && (
          <div className="member-contact" style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12.5 }}>
            ...
            <span style={{ color: "#475569" }}>{rep.contact}</span>
          </div>
        )}
      </div>
      <div style={{ display: "flex", gap: 8, marginTop: 2 }}>
        {rep.contact && (
          <a href={`tel:${rep.contact}`} className="call-blink"
        ...
        {waHref && (
          <a href={waHref} target="_blank" rel="noreferrer"
            style={{ flex: 1, display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "8px 10px", color: "#166534", fontSize: 12.5, fontWeight: 700, borderRadius: 9, textDecoration: "none", border: "1px solid #86EFAC", background: "#F0FDF4" }}>
...
function TeamGrid({ team, projectSlug }) {
  const hasRep = !!CLIENT_REPRESENTATIVES[projectSlug];
  if (!team?.length && !hasRep) return <p style={{ color: "#94A3B8", fontSize: 13 }}>Team details coming soon.</p>;
  return (
    <div className="team-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 14 }}>
      ...
      <ClientRepCard projectSlug={projectSlug} />
    </div>
  );
}
            ...
            <div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 18, alignItems: "stretch", marginBottom: 18 }} className="team-meeting-row">
                <SectionCard title="Your PFAS Advisory Team" style={{ marginBottom: 0 }}>
                  <TeamGrid team={filterTeamForProject(project.team, projectSlug)} projectSlug={projectSlug} />
                </SectionCard>
